...
public class AutoMLImageLabelerProcessor
    ...
    protected void onSuccess(
        ...
        try {
            ...
            int catsId = 0;
            FirebaseVisionImageLabel hologramImage = null;
            for (int i = 0; i < labels.size(); i++) {
                if (labels.get(i).getText().equals("cats")) {
                    catsId = i;
                }
            }
            labels.remove(catsId);
            if (labels.size() >= 1) {
                hologramImage = labels.get(0);
            }
            if (hologramImage != null) {
                labels.clear();
                labels.add(hologramImage);
